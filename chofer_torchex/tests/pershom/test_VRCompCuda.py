...
def test_l2_distance_matrix():
    ...
    d_ = __C.VRCompCuda__l2_norm_distance_matrix(point_cloud)
...
def test_l1_distance_matrix():
    ...
    d_ = __C.VRCompCuda__l1_norm_distance_matrix(point_cloud)

    for i, j in combinations(range(point_cloud.size(0)), 2):
        l1_dist = (point_cloud[i] - point_cloud[j]).abs().sum()
        l1_dist = float(l1_dist)

        assert float(d_[i, j]) == l1_dist
...
def test_VietorisRipsArgsGenerator__l1_excessive_state_testing_of_all_intermediate_steps(max_dimension,
                                                                                         max_ball_radius):
    ...
    assert testee.n_simplices_by_dim[1] == expected_n_dim_1_simplexes
    ...
    for dim in range(2, max_dimension + 1):
        n_dim_min_one_simplexes = testee.n_simplices_by_dim[dim - 1]
        ...
        for boundaries in combinations(range(n_dim_min_one_simplexes), dim + 1):
        ...
        for j, boundaries in enumerate(boundary_info.tolist()):
            ...
            for b in boundaries:
                assert testee.filtration_values_vector_without_vertices[int(b) - point_cloud.size(0)] <= \
                       testee.filtration_values_vector_without_vertices[id_]
    ...
    for i, boundary_info in enumerate(testee.boundary_info_non_vertices):
        ...
        for j, boundaries in enumerate(boundary_info.tolist()):
            ...
            assert testee.filtration_values_vector_without_vertices[id_] == max(filt_b)
    ...
    assert testee.boundary_array.size() == (sum(testee.n_simplices_by_dim[1:]),
                                            4 if max_dimension == 0 else 2 * (max_dimension + 1))
...
def test_VietorisRipsArgsGenerator__l1_valid_calculate_persistence_args(test_args):
    ...
    testee = pershom_backend.__C.VRCompCuda__VietorisRipsArgsGenerator()
    ba, ba_row_i_to_bm_col_i, simplex_dimension, sorted_filtration_values_vector = testee(distance_matrix,
                                                                                          max_dimension,
                                                                                          max_ball_radius)
