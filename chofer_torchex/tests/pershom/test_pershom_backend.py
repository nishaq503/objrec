...
class TestFindMergePairings:
    ...
    def test_result_1(self, device, dtype):
        ...
        expected_result = {(0, 5), (0, 6), (1, 2), (1, 3), (4, 8), (4, 9)}
...
class TestCalculatePersistence:
    ...
    @staticmethod
    def calculate_persistence_output_to_barcode_list(input_):
        ret = []
        b, b_e = input_
    ...
    def test_simple_1(self):
        ...
        out = pershom_backend.calculate_persistence(ba, ind_not_reduced, row_dim, max_dim_to_read_of_reduced_ba, 100)

        barcodes = TestCalculatePersistence.calculate_persistence_output_to_barcode_list(out)
    ...
    def test_random_simplicial_complexes(self):
        ...
        for sp_path in glob.glob('test_pershom_backend_data/random_simplicial_complexes/*'):
            ...
            ba, ind_not_reduced, row_dim = ba.to(device).type(dtype), ind_not_reduced.to(device).long(), row_dim.to(
                device).type(dtype)

            result = pershom_backend.calculate_persistence(ba, ind_not_reduced, row_dim, max_dim_to_read_of_reduced_ba,
                                                           10000)
            result = TestCalculatePersistence.calculate_persistence_output_to_barcode_list(result)
