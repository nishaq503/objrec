...
class Toplex:
    def __init__(self, toplexes: [tuple], filtration_values: [], deessentialize=False):
        self.simplexes = [tuple(t) for t in toplexes]
    ...
    def _check_state(self):
        if len(self.simplexes) != len(self.filtration):
            raise ToplexException("Simplexes and filtration are not consistent: Assumed to have same length.")
    ...
    def _simplex_to_string_iter(self):
        ...
        for s, f in zip(self.simplexes, self._internal_filt):
    ...
    def calculate_persistence_diagrams(self):
        ...
        homology_dimension_upper_bound = max([len(s) for s in self.simplexes])
...
def toplex_persistence_diagrams(toplexes: [tuple], filtration_values: [], deessentialize=False):
    """
    ...
    highest dimensional simplexes are noted, i.e. toplex
    {[1,2]} stands for the simplicial complex {[1], [2], [1,2]}

    :param toplexes: List of toplexes given as numeric tuples.
    The numeric value of each dimension of a toplex tuple stands
    for a vertex, e.g. [1, 2, 3] is the 2 simplex built from the vertices 1, 2, 3.

    :param filtration_values: List which gives the filtration value of each toplex
    enlisted in toplexes.
    ...
    :return: [[[]]
    """
    toplex = Toplex(toplexes, filtration_values, deessentialize=deessentialize)
