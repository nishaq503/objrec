...
def init_software_backends():
    ...
    if len(backend_errors) > 0:
        error_prefix = "The following backends are not properly configured\n"

        backend_errors = [b for b, _ in backend_errors]
        error_text = '\n'.join(backend_errors)

        error_text = error_prefix + error_text + '\n'
