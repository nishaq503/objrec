...
def _pack(value, type_: type):
    ...
    raise ValueError('Packing is not implemented for type {}.'.format(type_(value)))


def _unpack(bytes_to_unpack, type_: type):
    if type_ is float:
    ...
    raise ValueError('Unpacking is not implemented for type {}.'.format(type_))


def _unpack_from_file(f: io.BufferedReader, type_):
    read_bytes = f.read(8)
    return _unpack(read_bytes, type_)[0]


def _run_dipha(input_file, output_file,
...
def persistence_diagrams_of_filtrated_cubical_complex(filtrated_cubical_complex: numpy.array,
    ...
    :param set_inf_to_max_filt_val:
    :param benchmark:
    :param dual:
    :param limit_dimensions:
