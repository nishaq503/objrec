...
def is_prepared_batch(input_):
    if not (isinstance(input_, tuple) and len(input_) == 4):
        ...
        return all(
            (
                isinstance(batch, Tensor),
                isinstance(not_dummy_points, Tensor),
                max_points > 0,
                batch_size > 0,
            )
        )
...
def is_list_of_tensors(input_):
    try:
        return all([isinstance(x, Tensor) for x in input_])

    except TypeError:
        return False


def prepare_batch_if_necessary(input_, point_dimension=None):
    if is_prepared_batch(input_):
        batch, not_dummy_points, max_points, batch_size = input_
    elif is_list_of_tensors(input_):
        if point_dimension is None:
            point_dimension = input_[0].size(1)

        batch, not_dummy_points, max_points, batch_size = prepare_batch(input_, point_dimension)

    else:
        raise ValueError(f'SLayer does not recognize input format! Expecting [Tensor] or prepared batch. Not {input_}')
...
class SLayerExponential(Module):
    ...
    def forward(self, input_) -> Tensor:
        temp = prepare_batch_if_necessary(input_, point_dimension=self.point_dimension)
...
class SLayerRational(Module):
    ...
    def __init__(self, n_elements: int,
                 ...
                 point_wise_activation_threshold=None,
        ...
        self.point_wise_activation_threshold = float(point_wise_activation_threshold) \
            if point_wise_activation_threshold is not None else None
        self.share_sharpness = bool(share_sharpness)
        self.share_exponent = bool(share_exponent)
        self.freeze_exponent = freeze_exponent

        if self.point_wise_activation_threshold is not None:
            self.point_wise_activation_threshold = float(self.point_wise_activation_threshold)
        ...
        sharpness_init = parameter_init_from_arg(arg=sharpness_init,
                                                 size=(1,) if self.share_sharpness
                                                 else (self.n_elements, self.point_dimension),
                                                 default=torch.ones,
    ...
    def forward(self, input_) -> Tensor:
        ...
        if self.point_wise_activation_threshold is not None:
            x[(x < self.point_wise_activation_threshold)].data = 0
...
class SLayerRationalHat(Module):
    ...
    def forward(self, input_) -> Tensor:
        ...
        radius: Tensor = self.radius
        radius = radius.unsqueeze(1).expand(-1, max_points)
        radius = radius.unsqueeze(0).expand(batch_size, *radius.size())
        radius = radius.abs()

        norm_to_center: Tensor = centers - batch
        norm_to_center = torch.norm(norm_to_center, p=self.norm_p, dim=3)

        # noinspection PyUnresolvedReferences
        positive_part: Tensor = 1 / (1 + norm_to_center).pow(self.exponent)

        negative_part = 1 / (1 + (radius - norm_to_center).abs_()).pow(self.exponent)
