...
class Search:
    ...
    def linear_search(self, query: np.ndarray, radius: float) -> List[int]:
        ...
        for i in range(0, np.shape(self.data)[0], globals.BATCH_SIZE):
            end = min(i + globals.BATCH_SIZE, np.shape(self.data)[0])
    ...
    def clustered_search(self, query: np.ndarray, radius: float, search_depth: int) -> Tuple[List[int], int, float]:
        ...
        results = []
        for i in range(0, len(search_space), globals.BATCH_SIZE):
            end = min(i + globals.BATCH_SIZE, len(search_space))
